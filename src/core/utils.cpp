...
void logMemory(const iop::Log &logger) noexcept {
  IOP_TRACE();
  logger.info(F("Free Stack: "), std::to_string(driver::device.availableStack()),
              F(", Free Heap: "), std::to_string(driver::device.availableHeap()),
              F(", Biggest Heap Block: "), std::to_string(driver::device.biggestHeapBlock()));
}
} // namespace iop