...
public:
  ...
  auto registerEvent(const AuthToken &token, const Api::Json &event) const noexcept -> iop::NetworkStatus;
  ...
  /// BROKEN_SERVER: must wait until server is fixed
  auto update(const AuthToken &token) const noexcept -> iop_hal::UpdateStatus;
  ...
  auto makeJson(iop::StaticString contextName, Api::JsonCallback jsonObjectBuilder) const noexcept -> std::unique_ptr<Api::Json>;
};
