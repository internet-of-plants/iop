...
#define panic_(msg)                                                            \
  panic__(msg, StaticString(FPSTR(__FILE__)), (uint32_t)__LINE__,              \
          StringView(UnsafeRawString((const char *)__PRETTY_FUNCTION__)))
#define assert_(cond)                                                          \
  if (cond) {                                                                  \
    panic_(StringView(UnsafeRawString("Assertion failed: " #cond)));           \
  }
