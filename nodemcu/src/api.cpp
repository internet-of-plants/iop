...
Option<HttpCode> Api::reportPanic(const AuthToken &authToken,
  ...
  const auto makeJson = [](const Option<PlantId> &id, const PanicData &error) {
    auto doc = std::unique_ptr<StaticJsonDocument<2048>>(
        new StaticJsonDocument<2048>());
    if (id.isSome()) {
      const PlantId &idRef = id.asRef().expect(
          F("PlantId is None but shouldn't, at Api::reportPanic"));
...
Option<HttpCode> Api::registerEvent(const AuthToken &authToken,
  ...
  static const auto makeJson = [](const Log &logger, const Event &event) {
    ...
    auto doc =
        std::unique_ptr<StaticJsonDocument<256>>(new StaticJsonDocument<256>());
  ...
  return maybeResp.asRef().map<HttpCode>(
      [](const std::reference_wrapper<const Response> resp) {
...
Api::authenticate(const StringView username,
  ...
  const auto makeJson = [](const Log &logger, const StringView username,
  ...
  if (maybeResp.isNone()) {
    ...
    const Response &resp = maybeResp.asRef().expect(
        F("maybeResp is none, inside Api::authenticate"));
    ...
    return Result<AuthToken, Option<HttpCode>>(
        result.expectOk(F("result isn't Ok but should be")));
  }
#else
...
Option<HttpCode> Api::reportError(const AuthToken &authToken, const PlantId &id,
  ...
  const auto makeJson = [](const PlantId &id, const StringView error) {
    auto doc =
        std::unique_ptr<StaticJsonDocument<300>>(new StaticJsonDocument<300>());
...
Api::registerPlant(const AuthToken &authToken) const noexcept {
  const auto makeJson = [](const Api &api) {
    auto doc =
        std::unique_ptr<StaticJsonDocument<30>>(new StaticJsonDocument<30>());
  ...
  if (resp.code == 200) {
    ...
    return Result<PlantId, Option<HttpCode>>(
        result.expectOk(F("result isn't Ok but should be")));
