...
void Log::trace(const StaticString &msg, const enum LogType logType,
                const StaticString lineTermination) const noexcept {
  log(TRACE, msg, logType, lineTermination);
}

void Log::debug(const StaticString &msg, const enum LogType logType,
                const StaticString lineTermination) const noexcept {
  log(DEBUG, msg, logType, lineTermination);
}

void Log::info(const StaticString &msg, const enum LogType logType,
               const StaticString lineTermination) const noexcept {
  log(INFO, msg, logType, lineTermination);
}

void Log::warn(const StaticString &msg, const enum LogType logType,
               const StaticString lineTermination) const noexcept {
  log(WARN, msg, logType, lineTermination);
}

void Log::error(const StaticString &msg, const enum LogType logType,
                const StaticString lineTermination) const noexcept {
  log(ERROR, msg, logType, lineTermination);
}

void Log::crit(const StaticString &msg, const enum LogType logType,
               const StaticString lineTermination) const noexcept {
  log(CRIT, msg, logType, lineTermination);
}

void Log::trace(const StringView &msg, const enum LogType logType,
                const StaticString lineTermination) const noexcept {
  log(TRACE, msg.get(), logType, lineTermination);
}

void Log::debug(const StringView &msg, const enum LogType logType,
                const StaticString lineTermination) const noexcept {
  log(DEBUG, msg.get(), logType, lineTermination);
}

void Log::info(const StringView &msg, const enum LogType logType,
               const StaticString lineTermination) const noexcept {
  log(INFO, msg.get(), logType, lineTermination);
}

void Log::warn(const StringView &msg, const enum LogType logType,
               const StaticString lineTermination) const noexcept {
  log(WARN, msg.get(), logType, lineTermination);
}

void Log::error(const StringView &msg, const enum LogType logType,
                const StaticString lineTermination) const noexcept {
  log(ERROR, msg.get(), logType, lineTermination);
}

void Log::crit(const StringView &msg, const enum LogType logType,
               const StaticString lineTermination) const noexcept {
...
void Log::log(const enum LogLevel level, const char *const msg,
              const enum LogType logType,
              const StaticString lineTermination) const noexcept {
  if (this->logLevel > level)
    return;
