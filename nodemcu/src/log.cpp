...
void Log::log(const enum LogLevel level, const StaticString msg, const enum LogType logType, const StaticString lineTermination) const {
...
void Log::log(const enum LogLevel level, const char * const msg, const enum LogType logType, const StaticString lineTermination) const {
  if (this->logLevel > level) return;
  if (this->flush) Serial.flush();
  this->printLogType(logType, level);
  Serial.print(msg);
