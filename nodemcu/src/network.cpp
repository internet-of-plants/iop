...
Option<Response>
Network::httpRequest(const enum HttpMethod method,
  ...
  const auto methodString =
      methodToString(method).expect(F("HTTP method not recognized"));
  ...
  if (this->isConnected()) {
    ...
    if (token.isSome()) {
      const StringView &tok =
          token.asRef().expect(F("Missing auth token when it shouldn't"));
      http->addHeader(F("Authorization"), String("Basic ") + String(tok.get()));
    }

    const int httpCode = http->sendRequest(
        methodString.asCharPtr(),
        reinterpret_cast<const uint8_t *>(data_.get()), strlen(data_.get()));
